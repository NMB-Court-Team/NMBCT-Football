...
**相关文档**：[README.md](./README.md)（模组概览与按键）、[FOOTBALL_PHYSICS.md](./FOOTBALL_PHYSICS.md)（足球刚体物理、踢球/带球/守门员与对抗）、[FOOTBALL_MATCH.md](./FOOTBALL_MATCH.md)（比赛流程与体力 HUD）。近期平衡改动（滑铲、加速疾跑、视野外带球指示等）未改变球网实体协议与模拟模型；足球与球网的动量交换仍见下文 [足球交互](#足球交互)。
...
- **弹簧**：水平、竖直结构边（主约束）+ 二阶结构弹簧（`i±2` / `j±2`，弱约束，抑制条纹塌陷）+ 对角剪切弹簧（增强稳定性）。
...
2. `slack` 通过“重力增益 + 约束柔化”影响形态，不再直接放大弹簧静长，避免封闭大平面在高松弛度下起皱/山脊。
3. 多轮约束迭代：主结构 / 剪切 / 二阶弹簧使用不同刚度权重，兼顾平滑与稳定。
4. 约束采用“先累计、后统一应用”的批处理修正，并按弹簧刚度加权，降低固定遍历顺序偏置。
5. 每步后对非锚点节点执行“网节点-方块碰撞”修正：若进入方块碰撞体，沿 `+Y` 推出至方块表面上方，防止网面陷入地形。
6. 返回节点总位移平方和，用于静止判定。

`slack`（松弛度）越大，网越“重且软”，自然下垂越明显，同时保留网面的稳定性。
...
|------|------|
| `step(): Double` | 推进一步模拟 |
| `resolveBlockCollisions(level)` | 网节点与方块碰撞解算，防止节点嵌入地面/方块 |
...
1. 遍历实时变形网格的每个四边形单元（拆为两个三角形）。
2. 先做单元 AABB 与球体 swept AABB 粗筛，剔除不可能接触的单元。
3. 计算球心到三角形最近点，判定贴近（距离 < 球半径 + `CONTACT_MARGIN`）。
4. 结合球心线段与三角形平面相交测试判定穿面，优先处理穿面接触。
5. 仅在朝网运动或已穿透时生效。
...
> **注意**：碰撞检测已基于实时变形网格三角形；但仍是离散 tick 检测（非连续时间求解），极高速射门在极端角度下仍可能出现漏检。
...
| 常量 | 默认值 | 含义 |
|------|--------|------|
| `NET_BLOCK_COLLISION_RADIUS` | `0.06` | 节点与方块碰撞检测近似半径 |
| `NET_BLOCK_COLLISION_EPSILON` | `0.005` | 推出方块后的额外留缝，减少抖动/反复嵌入 |
...
1. **离散碰撞限制**：已使用变形网格三角形碰撞，且新增网节点-方块碰撞防陷地；但当前仍为离散 tick 检测，极高速、极薄擦边场景可能漏检。
...
- 引入连续碰撞检测（CCD）或分层加速结构（AABB/BVH）进一步提升高速场景稳健性与性能。
