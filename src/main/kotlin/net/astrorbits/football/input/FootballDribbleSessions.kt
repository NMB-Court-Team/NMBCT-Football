...
object FootballDribbleSessions {
    ...
    fun beginOrRefresh(player: ServerPlayer, football: Football, now: Long): Boolean {
        ...
        if (existing == null || existing.footballId != football.id) {
            sessions[player.uuid] = DribbleSession(
                ...
                nextSoundTick = now + resolveDribbleSoundIntervalTicks(player.random),
            )
    ...
    fun tick(server: MinecraftServer) {
        ...
        while (iterator.hasNext()) {
            ...
            if (FootballMovementInputUtil.hasMovementInput(player) && now >= session.nextSoundTick) {
    ...
    private fun resolveDribbleSoundIntervalTicks(random: RandomSource): Long {
        val base = FootballInputConfig.DRIBBLE_SOUND_INTERVAL_BASE_TICKS
