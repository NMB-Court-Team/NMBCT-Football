...
class FreeKickAwardHudElement : HudElement {
    ...
    override fun extractRenderState(extra: GuiGraphicsExtractor, delta: DeltaTracker) {
        ...
        val elapsedMs = FreeKickAwardClient.displayElapsedMs
        val foulPlayerColor = MatchEventBanner.teamColor(FreeKickAwardClient.foulingTeam)

        if (FreeKickAwardClient.displayMode == FreeKickAwardClient.DisplayMode.FOUL_ONLY) {
            MatchEventBanner.renderFoul(
                extra = extra,
                font = client.font,
                ...
                elapsedMs = elapsedMs,
                durationMs = 4000L,
                headlineText = Component.translatable("hud.nmbct-football.foul.headline").string,
                foulLine = foulLine,
                foulPlayerColor = foulPlayerColor,
            )
            return
        }
